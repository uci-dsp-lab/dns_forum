...
class Crawler_github:
    ...
    def start_crawling(self, current_url, pageNum, closed):
        ...
        db = client.github_crawled_data_PowerDNS
    ...
    def extract_next_urls(self, starting_url):
        ...
        visited, required, base_url = defaultdict(int), "/PowerDNS/pdns/issues/", "https://github.com/PowerDNS/pdns/issues/"
        urlcontainer = soup.findAll("a")
        for anchor in urlcontainer:
            new_url = anchor.get("href")

            if visited[new_url] == 1 or required not in new_url or "?" in new_url:
                continue
...
if __name__ == "__main__":
    ...
    base_url = "https://github.com/PowerDNS/pdns/issues?page=10&q=is%3Aissue+is%3Aclosed"
    # for pageNum in range(1, 10): # Closed posts in the last year

    #pageNum for crawling
    for pageNum in range(1, 2):
        url = "https://github.com/PowerDNS/pdns/issues?page={0}&q=is%3Aissue+is%3Aclosed".format(pageNum)
