...
#  - store_crawled_data(url_response, id, slug, real_url): Storing the crawled data on Mongodb.
#  - extract_next_urls(json_url): Taking in an URL which contains URL information of all the posts on that page in JSON format,
#  return those URLs.
    ...
    def external_crawl(self, included_url):
        ...
        for ending in bList:
            if ending in included_url:
                return
        if "http" not in included_url or self.already_met[included_url] == 1:
            return
